...
//  fFiber_vert_dis = 1.22 + 0.0626;
  ...
  G4double fGlue_thickness = 0.504;  // 0.233; // Expected value as in Glue-X. TBD more
...
/*
    G4Box* Acrylic = new G4Box("Acrylic1", (fModuleH / 2.) * mm, (fModuleW * 2.) * mm,
                       (fTowerDepth / 2.) * mm);
    G4LogicalVolume* AcrylicLogical =
        new G4LogicalVolume(Acrylic, FindMaterial("PMMA"), "51");

    new G4PVPlacement(0, G4ThreeVector(-75*mm,1060*mm,0*mm), AcrylicLogical, "51",
                      worldLogical, false, 0, checkOverlaps);
  ...
  if (doFiber) {
    for (unsigned int fiberId = 0; fiberId < fFiberX.size(); fiberId++) {
      ...
      fiberClad2Intersection__[i].push_back(new G4LogicalVolume(
          tfiberClad2Intersection, FindMaterial("G4_Galactic"), name));
      new G4PVPlacement(
...
logicAir2 -> SetVisAttributes(fVisAttrBlue);     
  ...
  G4VSolid *space3 = new G4Box("glue4", (0.504/ 2.) * mm,
                    (0.175 / 2.) * mm, (320. / 2.) * mm);
  ...
  G4VSolid *space2 = new G4Box("glue3", (0.504 / 2.) * mm,
                    (0.3 / 2.) * mm, (320. / 2.) * mm);
