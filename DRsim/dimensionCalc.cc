...
G4ThreeVector dimensionCalc::GetOrigin(G4int i) {
  ...
  if (col < 1) { 
   return G4ThreeVector( -fmodule_height * (double)fNofRow/2. + row * fmodule_height + fmodule_height/2., -fmodule_width * (double)fNofCol/2. + col * fmodule_width + fmodule_width/2. + fFrontL + fNofCol * fmodule_width/2. - 30*vac  , 0.);
}
  else{
