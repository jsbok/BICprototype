...
//  TString filename1 = "/u/user/changhui/koBIC2026/BICprototype/rootfiles/3SF1B_e-_1_GeV_2026/root/R3SF1B_e-_1_GeV_2026_500";
  ...
  TString filename1 = "/home/changhui/work/BICprototype2026/TBCERN2026b/build/koBIC/_0";
...
for (int i =1; i < 44; i++){
  ...
  TH1F* Nhits_M_L[44]; TH1F* Nhits_M_R[44];
for (int j =1; j < 44; j++){
  ...
  TH1F* THits = new TH1F("THits",";ModuleN; # of p.e (%)",43,0,43);
  THits->Sumw2(); THits->SetLineColor(kRed); THits->SetLineWidth(2);
  TH1F* THits2 = new TH1F("THits2",";ModuleN; # of p.e.",43,0,43);
  THits2->Sumw2(); THits2->SetLineColor(kBlue); THits2->SetLineWidth(2);
 
  TH1F* THitss = new TH1F("THits",";ModuleN; Edep(MeV)",43,0,43);
  THitss->Sumw2(); THitss->SetLineColor(kRed); THitss->SetLineWidth(2);
  TH1F* THitss2 = new TH1F("THits2",";ModuleN; # of p.e.",43,0,43);
...
float Edep_Num[43] = {0};float Edep_Num2[43] = {0}; float Edep_Numm[43] = {0};

if (file1==1) {

 TFile *file = new TFile("/home/changhui/work/BICprototype2026/TBCERN2026b/build/analysis/1GeV_tree.root", "RECREATE");
 TTree *tree = new TTree("Ttree", "Edep and Nhits ");
// tree->SetBranchAddress("edep.Edep", &edep);
// tree->SetBranchAddress("edep.Module", &moduleN);

    Float_t edep1[43]={0};  // 24개의모듈 데이터를 저장
    Int_t moduleN_E[43]={0} ; Int_t moduleN_H[43]={0} ; Float_t Nhits_L[43]={0}; Float_t Nhits_R[43]={0}; Float_t Core_E[43]={0};
    ...
    float totHit = 0; float nHits[43]= {0};
...
//    float Nhits_L[25]={0}; float Nhits_R[25]={0};  int moduleN_E[25]={0} ; int moduleN_H[25]={0};
    ...
    float EdepCore = 0.;float Edep = 0.; float totE = 0.; float ratE = 0;  float Edep_Num[43] = {0}; float EdepC_Num[43] = {0};

 float Nhits_Numm_L[43]={0}; float Nhits_Numm_R[43]={0};
...
//std::cout<<"Edep_Numm : "<<Edep_Num[j-1]<< "edep : " << edep1[j] << "moduleN : "<< moduleN_E[j] <<  j <<std::endl;
    ...
    for (int z=0;z<43;z++) {  THits -> Fill(z,nHits[z]*100/totHit); }
...
//    std::cout << "4GeV BinErr = " << tEdep2->GetStdDev()*100/e2  << std::endl;
 ...
 for (int ii=1; ii<44; ii++){
  ...
  std::cout << "Module total1 = " << THitss->Integral() << std::endl;
    ...
    TF1 *gaussFit_E = new TF1("gaussFit_E", "gaus", 0, 4300);
...
//
 ...
 for (int o=1; o<44; o++){ float val = THitss->GetBinContent(o); 
...
*/
   ...
   TFile *outputFile2 = new TFile("/home/changhui/work/BICprototype2026/TBCERN2026b/build/analysis/1GeV_hist.root", "RECREATE");
tEdep->Write(); tEdepC->Write(); tHit_S->Write();
for (int jjj=1; jjj<44; jjj++){ 
    Edep_M[jjj]->Write();
}
for (int jjjj=1; jjjj<44; jjjj++){
...
for (int j=1; j<44; j++){   //  if (j<1 || j>43) continue;
