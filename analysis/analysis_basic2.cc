...
int main(int argc, char* argv[]) {
  TString filename1 = "/u/user/changhui/work/BICprototype/rootfiles/3by5_e-_3_GeV_final/root/R3by5_e-_3_GeV_final_10M";
  TString filename2 = "/u/user/changhui/work/BICprototype/rootfiles/3by8_e-_3_GeV_final/root/R3by8_e-_3_GeV_final_10M" ;
  TString filename3 = "/u/user/changhui/work/BICprototype/rootfiles/3by11_e-_3_GeV_final/root/R3by11_e-_3_GeV_final_10M" ;
  TString filename4 = "/u/user/changhui/work/BICprototype/rootfiles/5by8_e-_3_GeV_final/root/R5by8_e-_3_GeV_final_10M";
  TString filename5 = "/u/user/changhui/work/BICprototype/rootfiles/2by8_e-_3_GeV_final/root/R2by8_e-_3_GeV_final_10M" ;
  TString filename6 = "/u/user/changhui/work/BICprototype/rootfiles/4by8_e-_3_GeV_final/root/R4by8_e-_3_GeV_final_10M" ;
  int file1=1; int file2=1; int file3=1; int file4=0; int file5=1; int file6=1; // File loop on/off

  float inE = std::stof(argv[1]);
  float low = std::stof(argv[2]);
  float high = std::stof(argv[3]);

  int e1 = inE*3;int e2 = inE*3; int e3 = inE*3 ; int e4 = inE*3 ; int e5 = inE*3; int e6 = inE*3; // Beam E of each file
  int nLayers = 22;  int nFibers = 24;  int nRows = 3;  int nColumns = 8; // module configuration
  ...
  TH1F* THitss = new TH1F("THits",";ModuleN; Edep(MeV)/evt",20,0,20);
  THitss->Sumw2(); THitss->SetLineColor(kRed); THitss->SetLineWidth(2);
  TH1F* THitss2 = new TH1F("THits2",";ModuleN; # of p.e.",20,0,20);
  THitss2->Sumw2(); THitss2->SetLineColor(kBlue); THitss2->SetLineWidth(2);
  TH1F* THitss3 = new TH1F("THits3",";ModuleN; # of p.e.",20,0,20);
...
float Edep_Num[16] = {0};float Edep_Num2[16] = {0};float Edep_Num3[16] = {0};float Edep_Num4[16] = {0};
...
/*
    for (int ii=1; ii<17; ii++){
     THitss->SetBinContent(ii,Edep_Num[ii-1]);
    } 
*/
...
if (file6==1) {
 ...
 for (int ii=1; ii<17; ii++){
     THitss->SetBinContent(ii,Edep_Num[ii-1]/10000);
    }
 ...
 for (int ii4=1; ii4<17; ii4++){
  ...
  std::cout << "Module total1 = " << THitss->Integral() << std::endl;
   ...
   Edep_ratio5 -> SetBinContent(30,rat_E5*100);
   Edep_ratio5 -> SetBinError(30, tEdep5->GetStdDev()*100/e5);
...
*/
  ...
  tEdep->Draw("Hist");  tEdep2->Draw("Hist same"); tEdep3->Draw("Hist same");  tEdep4->Draw("Hist same"); tEdep5->Draw("Hist same"); 
//tEdep->SetStats(0);
c->SaveAs(filename1+"compare_Edep_e.png");   
...
c->SaveAs(filename1+"compare_Pleak_e.png");
 ...
 c->SaveAs(filename1+"compare_towerE_e.png");
  ...
  THitss->Draw("Hist"); THitss2->Draw("Hist same"); THitss3->Draw("Hist same");  THitss4->Draw("Hist same"); THitss5->Draw("Hist same");
  THitss->SetStats(0);
 c->SaveAs(filename1+"compare_tower_Edep.png");

   tHit_S->Draw("Hist"); tHit_S2->Draw("Hist same");tHit_S3->Draw("Hist same"); tHit_S4->Draw("Hist same"); tHit_S5->Draw("Hist same"); c->SaveAs(filename1 +"compare_nHitpEventS_e.png");
...
Edep_ratio->SetMarkerSize(1.3); Edep_ratio->SetMarkerStyle(20); Edep_ratio->GetYaxis()->SetRangeUser(80,100); Edep_ratio->SetMarkerColor(2);
