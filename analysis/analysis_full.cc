...
int main(int argc, char* argv[]) {
  ...
  int file1=1; int file2=1; int file3=1; int file4=1; int file5=0; int file6=0; // File loop on/off

  float inE = std::stof(argv[1]);
  float low = std::stof(argv[2]);
  float high = std::stof(argv[3]);

  int e1 = inE*3;int e2 = inE*3; int e3 = inE *3 ; int e4 = inE*3 ; int e5 = inE*3; int e6 = inE/2; // Beam E of each file
...
*/
  TH1F* Edep_ratio = new TH1F("Edep_ratio","; x (mm); Edep (%)",32,-16.,16.);
...
//    tHit_S_right->Fill(nHitS_right);
    ...
    Edep_ratio -> SetBinContent(21,rat_E*100);
    float a = Edep_ratio->GetBinContent(21);
    std::cout << "BinCon = " << a << std::endl;
    Edep_ratio -> SetBinError(21, tEdep->GetStdDev()*100/e1);
...
//    tHit_S_right2->Fill(nHitS_right2);
    ...
    Edep_ratio -> SetBinContent(17,rat_E2*100);
    Edep_ratio -> SetBinError(17, tEdep2->GetStdDev()*100/e2);
...
//  leak_ratio3 -> Fill(15,l3/50);
   ...
   Edep_ratio -> SetBinContent(25,rat_E3*100);
   Edep_ratio -> SetBinError(25, tEdep3->GetStdDev()*100/e3);
   rat_E4 = tEdep4->GetMean() / e3 ;
   Edep_ratio4 -> SetBinContent(29,rat_E4*100);
   Edep_ratio4 -> SetBinError(29, tEdep4->GetStdDev()*100/e3);
 ...
 */
  ...
  tEdep->Draw("Hist");  tEdep2->Draw("Hist same");
tEdep3->Draw("Hist same");  tEdep4->Draw("Hist same"); tEdep5->Draw("Hist same"); c->SaveAs(filename1+"compare_pos_Edep_e.png");   
  c->SetLogy(1);
  tP_leak->Draw("Hist"); tP_leak2->Draw("Hist same");tP_leak4->Draw("Hist same"); tP_leak4->Draw("Hist same"); tP_leak3->Draw("Hist same"); 
c->SaveAs(filename1+"compare_pos_Pleak_e.png");
