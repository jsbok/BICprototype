...
void koBICDetectorConstruction::FiberImplement(
 ...
 auto FillFibersInMother = [&](G4LogicalVolume* motherLog, G4double thickness, G4VSolid* cladSolid, G4VSolid* coreSolid) {
    ...
    for (int k = 0; k < numRows; k++) {
        ...
        for (int j = 0; j < numCols; j++) {
            ...
            new G4PVPlacement(fiberRot, G4ThreeVector(localH, 0, localV), cladLog,
            ...
            new G4PVPlacement(0, G4ThreeVector(0, 0, 0), coreLog,
                ...
                if (doGlue) {
                    ...
                    tGlueIntersection = new G4IntersectionSolid("glue", tGlueSubtraction, cladSolid, 0, G4ThreeVector(0,0,0));
    ...
    for (auto pbLog : logicPbSFIL) {
        FillFibersInMother(pbLog, 21.73*mm, fiberClad_SFIL, fiberCore_SFIL);
    }
    for (auto pbLog : logicPbBulk) {
        FillFibersInMother(pbLog, 20.889*mm, fiberClad_Bulk, fiberCore_Bulk);
    }
}
